...
class Node:
    ...
    def get_children(self, minimizing_player, mandatory_jumping):
        ...
        for i in range(len(available_moves)):
            ...
            Checkers.make_a_move(state, old_i, old_j, new_i,
            ...
            if len(old) != 2 or len(new) != 2:
                ...
                if not old_i.isdigit() or not old_j.isdigit() or not new_i.isdigit() or not new_j.isdigit():
                    ...
                    if move not in available_moves:
                        ...
                        Checkers.make_a_move(self.matrix, int(old_i), int(
                            old_j), int(new_i), int(new_j), "B", 0)
                        for m in range(8):
                            for n in range(8):
    ...
    def evaluate_states(self):
        ...
        if len(first_computer_moves) == 0:
            if self.player_pieces > self.computer_pieces:
                ...
                print(
                    ansi_yellow + "Computer has no available moves left.\nGAME ENDED!" + ansi_reset)
                exit()
        dict = {}
    ...
    def play(self):
        ...
        while True:
            ...
            if answer == "Y" or answer == "y":
                ...
                print(
                    ansi_cyan + "You've surrendered before the game even started.\nPathetic." + ansi_reset)
                exit()
            else:
                print(ansi_red + "Illegal input!" + ansi_reset)
        while True:
